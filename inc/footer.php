<footer class="footer">
    <div class="container">
        <div class="col-xl-11 col-lg-12  mx-auto">
            <div class="row">
                <div class="col-lg-5 col-md-12">
                    <div class="footer_about">
                        <div class="logo">
                            <figure>
                                <img src="img/footer_logo.png" class="img-fluid" alt="">
                            </figure>
                            <div class="footer_text">
                                <h5 class="mt-4">About</h5>
                                <p>Lorem ipsum dolor sit amet, consectetur adipiscing <br> elit, sed do eiusmod tempor
                                    incididunt ut labore.</p>
                                <ul class="social_icons">
                                    <li><a href=""><img src="img/insta.png" alt=""></a></li>
                                    <li><a href=""><img src="img/yt.png" alt=""></a></li>
                                    <li><a href=""><img src="img/fb.png" alt=""></a></li>
                                    <li><a href=""><img src="img/unknown.png" alt=""></a></li>
                                    <li><a href=""><img src="img/tw.png" alt=""></a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-7 col-md-12">
                    <div class="row">
                        <div class="col-md-4">
                            <div class="footer_links">
                                <div class="heading">
                                    <h5>Menu</h5>
                                </div>
                                <ul>
                                    <li><a href="">Home</a></li>
                                    <li><a href="">About</a></li>
                                    <li><a href="">Services</a></li>
                                    <li><a href="">Book Now</a></li>
                                    <li><a href="">Contact Us</a></li>
                                </ul>
                            </div>
                        </div>
                        <div class="col-md-4 ps-0">
                            <div class="footer_links">
                                <div class="heading">
                                    <h5>Important links</h5>
                                </div>
                                <ul>
                                    <li><a href="privacy-policy.php">Privacy Policy</a></li>
                                    <li><a href="terms-condition.php">Terms and Conditions</a></li>
                                    <li><a href="cancellation-policy.php">Cancelation Policy</a></li>
                                    <li><a href="booking-fees.php">Booking Fees</a></li>
                                    <li><a href="payment-agreement.php">Payment Agreement</a></li>
                                    <li><a href="faqs.php">FAQ</a></li>
                                </ul>
                            </div>
                        </div>
                        <div class="col-xl-3 col-lg-4">
                            <div class="footer_links address">
                                <div class="heading">
                                    <h5>Contact</h5>
                                </div>
                                <ul>
                                    <li class="mt-4 pt-2">
                                        <a href="">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="12.459" height="22.084"
                                                viewBox="0 0 15.459 22.084">
                                                <path id="Icon_material-location-on"
                                                    data-name="Icon material-location-on"
                                                    d="M15.23,3A7.724,7.724,0,0,0,7.5,10.73c0,5.8,7.73,14.355,7.73,14.355s7.73-8.558,7.73-14.355A7.724,7.724,0,0,0,15.23,3Zm0,10.49A2.761,2.761,0,1,1,17.99,10.73,2.762,2.762,0,0,1,15.23,13.49Z"
                                                    transform="translate(-7.5 -3)" fill="#fff"></path>
                                            </svg>
                                            <span>79 Branch Street
                                                Lemont, IL 60439</span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="18.301" height="18.301"
                                                viewBox="0 0 18.301 18.301">
                                                <path id="Icon_awesome-phone-alt" data-name="Icon awesome-phone-alt"
                                                    d="M.522,12.933l4-1.716a.858.858,0,0,1,1,.247L7.3,13.63A13.249,13.249,0,0,0,13.633,7.3L11.467,5.523a.856.856,0,0,1-.247-1l1.716-4a.864.864,0,0,1,.983-.5L17.636.88a.858.858,0,0,1,.665.836A16.584,16.584,0,0,1,1.716,18.3a.858.858,0,0,1-.836-.665L.021,13.92a.868.868,0,0,1,.5-.987Z"
                                                    transform="translate(0 0)" fill="#fff"></path>
                                            </svg>
                                            <span>202-555-0151</span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="">
                                            <img src="img/Icon zocial-email.png" alt="">
                                            <span>Info@babysitterworld.com</span>
                                        </a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="copy_right">
                <p>Copyrights © 2022 Babysitter World. All rights reserved.</p>
            </div>
        </div>
    </div>
</footer>
<?php include 'footerlinks.php'?>